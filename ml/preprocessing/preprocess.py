...
def preprocess(
    ...
    report = {
        ...
        "split_rating_distribution": {
            "train": {str(key): int(value) for key, value in train_df["rating"].value_counts().sort_index().to_dict().items()},
            "validation": {
                str(key): int(value) for key, value in valid_df["rating"].value_counts().sort_index().to_dict().items()
            },
            "test": {str(key): int(value) for key, value in test_df["rating"].value_counts().sort_index().to_dict().items()},
        },
